...
from _cosine_k_means_common import CHUNK_SIZE
from _cosine_k_means_common cimport _relocate_empty_clusters_dense
from _cosine_k_means_common cimport _relocate_empty_clusters_sparse
from _cosine_k_means_common cimport _average_centers, _center_shift


def cosine_lloyd_iter_chunked_dense(
...
cdef void _cosine_update_chunk_dense(
        const floating[:, ::1] X,                   # IN
        const floating[::1] sample_weight,          # IN
        const floating[:, ::1] centers_old,         # IN
    ...
    # CosineDistance(X, C) = 1 - X.C^T / ||X||.||C||, we only need to store
    # the - X.C^T / ||C|| term since the argmin for a given sample only
    ...
    # pairwise_distances += -1 * X.dot(C.T)
    ...
    for i in range(n_samples):
        for j in range(n_clusters):
            pairwise_distances[i * n_clusters + j] /= sqrt(centers_squared_norms[j])
...
def cosine_lloyd_iter_chunked_sparse(
        X,                                   # IN
        const floating[::1] sample_weight,   # IN
        const floating[:, ::1] centers_old,  # IN
        floating[:, ::1] centers_new,        # OUT
...
cdef void _cosine_update_chunk_sparse(
    ...
    for i in range(n_samples):
        ...
        for j in range(n_clusters):
            ...
            # the - X.C^T / ||C|| term since the argmin for a given sample only
            # depends on the centers.
            sq_dist = (-1 * sq_dist) / sqrt(centers_squared_norms[j])
            if sq_dist < min_sq_dist:
                min_sq_dist = sq_dist
                label = j
